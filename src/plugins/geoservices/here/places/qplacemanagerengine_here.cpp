...
#include "places/qplacecategoriesreplyimpl.h"
#include "places/qplacecontentreplyimpl.h"
#include "places/qplacesearchsuggestionreplyimpl.h"
#include "places/qplacesearchreplyimpl.h"
#include "places/qplacedetailsreplyimpl.h"
#include "places/qplaceidreplyimpl.h"
...
static const char * const HereIcon = "hereIcon";
static const char * const HereIconGenerated = "hereIconGenerated";
...
QPlaceManagerEngineHere::QPlaceManagerEngineHere(
    ...
    , m_uriProvider(new QGeoUriProvider(this, parameters, "places.host", PLACES_HOST))
{
    ...
    m_appCode = parameters.value(QLatin1String("app_code")).toString();
    ...
    if (m_localDataPath.isEmpty()) {
        ...
        if (!dataLocations.isEmpty() && !dataLocations.first().isEmpty()) {
            m_localDataPath = dataLocations.first()
                                + QLatin1String("/QtProject/QtLocation/data");
        }
    }
...
QPlaceDetailsReply *QPlaceManagerEngineHere::getPlaceDetails(const QString &placeId)
{
...
QPlaceContentReply *QPlaceManagerEngineHere::getPlaceContent(const QPlaceContentRequest &request)
{
    ...
    if (request.contentContext().userType() == qMetaTypeId<QUrl>()) {
       ...
       networkReply = sendRequest(u);
        ...
        QUrl requestUrl(QString::fromLatin1("http://") + m_uriProvider->getCurrentHost() +
                        QLatin1String("/places/v1/places/") + request.placeId() + QLatin1String("/media/"));
...
QPlaceSearchReply *QPlaceManagerEngineHere::search(const QPlaceSearchRequest &query)
{
...
QPlaceSearchSuggestionReply *QPlaceManagerEngineHere::searchSuggestions(const QPlaceSearchRequest &query)
{
    bool unsupported = false;
...
QPlaceIdReply *QPlaceManagerEngineHere::savePlace(const QPlace &place)
{
    QPlaceIdReplyImpl *reply = new QPlaceIdReplyImpl(QPlaceIdReply::SavePlace, this);
    reply->setId(place.placeId());
    QMetaObject::invokeMethod(reply, "setError", Qt::QueuedConnection,
                              Q_ARG(QPlaceReply::Error, QPlaceReply::UnsupportedError),
                              Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, SAVING_PLACE_NOT_SUPPORTED)));
    connect(reply, SIGNAL(finished()), this, SLOT(replyFinished()));
    connect(reply, SIGNAL(error(QPlaceReply::Error,QString)),
            this, SLOT(replyError(QPlaceReply::Error,QString)));
    return reply;
...
QPlaceIdReply *QPlaceManagerEngineHere::removePlace(const QString &placeId)
{
    QPlaceIdReplyImpl *reply = new QPlaceIdReplyImpl(QPlaceIdReply::RemovePlace, this);
    reply->setId(placeId);
    QMetaObject::invokeMethod(reply, "setError", Qt::QueuedConnection,
                              Q_ARG(QPlaceReply::Error, QPlaceReply::UnsupportedError),
                              Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, REMOVING_PLACE_NOT_SUPPORTED)));
    connect(reply, SIGNAL(finished()), this, SLOT(replyFinished()));
    connect(reply, SIGNAL(error(QPlaceReply::Error,QString)),
            this, SLOT(replyError(QPlaceReply::Error,QString)));
    return reply;
...
QPlaceIdReply *QPlaceManagerEngineHere::saveCategory(const QPlaceCategory &category, const QString &parentId)
{
    Q_UNUSED(parentId)
    ...
    QMetaObject::invokeMethod(reply, "setError", Qt::QueuedConnection,
                              Q_ARG(QPlaceReply::Error, QPlaceReply::UnsupportedError),
                              Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, SAVING_CATEGORY_NOT_SUPPORTED)));
    connect(reply, SIGNAL(finished()), this, SLOT(replyFinished()));
    connect(reply, SIGNAL(error(QPlaceReply::Error,QString)),
            this, SLOT(replyError(QPlaceReply::Error,QString)));
    return reply;
...
QPlaceIdReply *QPlaceManagerEngineHere::removeCategory(const QString &categoryId)
{
    QPlaceIdReplyImpl *reply = new QPlaceIdReplyImpl(QPlaceIdReply::RemoveCategory, this);
    reply->setId(categoryId);
    QMetaObject::invokeMethod(reply, "setError", Qt::QueuedConnection,
                              Q_ARG(QPlaceReply::Error, QPlaceReply::UnsupportedError),
                              Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, REMOVING_CATEGORY_NOT_SUPPORTED)));
    connect(reply, SIGNAL(finished()), this, SLOT(replyFinished()));
    connect(reply, SIGNAL(error(QPlaceReply::Error,QString)),
            this, SLOT(replyError(QPlaceReply::Error,QString)));
    return reply;
...
QPlaceReply *QPlaceManagerEngineHere::initializeCategories()
{
    if (m_categoryReply)
...
QString QPlaceManagerEngineHere::parentCategoryId(const QString &categoryId) const
{
    return m_categoryTree.value(categoryId).parentId;
}

QStringList QPlaceManagerEngineHere::childCategoryIds(const QString &categoryId) const
{
    return m_categoryTree.value(categoryId).childIds;
}

QPlaceCategory QPlaceManagerEngineHere::category(const QString &categoryId) const
{
    return m_categoryTree.value(categoryId).category;
}

QList<QPlaceCategory> QPlaceManagerEngineHere::childCategories(const QString &parentId) const
{
...
QList<QLocale> QPlaceManagerEngineHere::locales() const
{
    return m_locales;
}

void QPlaceManagerEngineHere::setLocales(const QList<QLocale> &locales)
{
    m_locales = locales;
}

QPlaceIcon QPlaceManagerEngineHere::icon(const QString &remotePath,
                                            const QList<QPlaceCategory> &categories) const
{
    ...
    if (rx.indexIn(remotePath) != -1 && !rx.cap(1).isEmpty() && !rx.cap(2).isEmpty()) {
        ...
        if (QFile::exists(m_localDataPath + hereIcon))
            iconPrefix = QString::fromLatin1("file://") + m_localDataPath;

        params.insert(HereIcon, hereIcon);
        params.insert(IconPrefix, iconPrefix);

        foreach (const QPlaceCategory &category, categories) {
            if (category.icon().parameters().value(HereIcon) == hereIcon) {
                params.insert(HereIconGenerated, true);
                break;
            }
        }
    } else {
...
QUrl QPlaceManagerEngineHere::constructIconUrl(const QPlaceIcon &icon,
                                                        const QSize &size) const
{
    Q_UNUSED(size)
    QVariantMap params = icon.parameters();
    QString hereIcon = params.value(HereIcon).toString();

    if (!hereIcon.isEmpty()) {
        hereIcon.append(!m_theme.isEmpty() ?
                         QLatin1Char('.') + m_theme : QString());
...
void QPlaceManagerEngineHere::replyFinished()
{
...
void QPlaceManagerEngineHere::replyError(QPlaceReply::Error error_, const QString &errorString)
{
    QPlaceReply *reply = qobject_cast<QPlaceReply *>(sender());
    if (reply)
...
void QPlaceManagerEngineHere::categoryReplyFinished()
{
        ...
        if (!document.isObject()) {
            if (m_categoryReply) {
                QMetaObject::invokeMethod(m_categoryReply.data(), "setError", Qt::QueuedConnection,
                                          ...
                                          Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, PARSE_ERROR)));
            }
            return;
        }
...
void QPlaceManagerEngineHere::categoryReplyError()
{
    if (m_categoryReply) {
        QMetaObject::invokeMethod(m_categoryReply.data(), "setError", Qt::QueuedConnection,
                                  Q_ARG(QPlaceReply::Error, QPlaceReply::CommunicationError),
                                  Q_ARG(QString, QCoreApplication::translate(HERE_PLUGIN_CONTEXT_NAME, NETWORK_ERROR)));
    }
}

QNetworkReply *QPlaceManagerEngineHere::sendRequest(const QUrl &url)
{
...
QByteArray QPlaceManagerEngineHere::createLanguageString() const
{
    QByteArray language;
